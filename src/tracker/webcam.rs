...
//!
//! Algorithm: poll the camera, run YuNet on the frame, triangulate Z
//! from the inter-pupillary distance, deproject (X, Y) through pinhole.
...
impl WebcamBackend {
    ...
    fn frame_to_pose(&mut self, frame: &RgbFrame) -> Option<Pose> {
        ...
        let y_mm = (v - self.cy) * z_mm / self.fx;
